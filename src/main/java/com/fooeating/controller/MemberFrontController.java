...
public class MemberFrontController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * 패턴1 : 처리작업 x (DB사용x), view 페이지(.foo와 연결된) 이동 패턴2 : 처리작업 o (DB사용o), 페이지(전혀 다른
		 * 페이지) 이동 처리작업 o (DB사용x), 페이지(전혀 다른 페이지) 이동 패턴3 : 처리작업 o (DB사용o), view
		 * 페이지(.foo와 연결된) 이동 + 출력
		 */
		...
		// 9-1 회원 대기 취소
		else if (command.equals("/MemberCancelWaiting.foo")) {
			System.out.println(" C : /MemberCancelWaiting.foo 실행");
			System.out.println("패턴2");
			
			
			// MemberCancelWaiting() 객체 생성
			
			action = new MemberCancelWaiting();
		...
		else if(command.equals("/crewInformation.foo")) {
			System.out.println("C : companyInformation.foo 호출");
		    System.out.println(" 패턴1 view페이지 이동");
		    
		    forward = new ActionForward();
			forward.setPath("./main/crewInformation.jsp");
			forward.setRedirect(false);
