	...
	<main>
		<div id="top">
			<div id="category"
					...
					<tbody>
						<c:forEach var="dto" items="${restWaitList}">
							<tr>
								...
								<td>${dto.regdate}</td>
								<td>
									<button type="submit" onclick="winopen('${dto.rest_id}');"
										class="btn-2">승인/반려</button>
								</td>
								<td>
									<form action="./RestaurantInfo.ad" method="post">
										<input type="hidden" name="pageNum" value="${pageNum}">
										<input type="hidden" name="rest_id" value="${dto.rest_id}">
										<input type="hidden" name="rstatus" value="no"> <input
				...
				%>
				<a href="./RestaurantWaitList.ad?pageNum=<%=i%>">[<%=i%>]
				</a>
				<%
				}
				if (endPage < pageCount) {
